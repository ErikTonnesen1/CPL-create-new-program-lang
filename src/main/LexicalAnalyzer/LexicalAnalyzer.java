...
package src.main.LexicalAnalyzer;

import java.util.*;

import src.main.Token.Token;
import src.main.Token.Token.Type;
...
public class LexicalAnalyzer {
    ...
    // when a Lexical analyzer is initialized, it creates all tokens from source code to be ready for a parser's getToken call
     public void findAllTokens(){
        ...
        for(int i =this.index; i < this.source.length(); i++){
            ...
            if (this.source.charAt(i) == ';'){
                ...
                //    throw new IllegalArgumentException("Improper end of line at row: 0 col: " + i);
                //}else {
                ...
                if (i != this.source.length() - 1 && Character.isDigit(this.source.charAt(i + 1))) {
                    ...
                    while ((findLastDigit + 1) <= this.source.length() - 1 && Character.isDigit(this.source.charAt(findLastDigit))) {
                        findLastDigit++;
                    }
    ...
    private void createToken(String Lexeme, int row, int column) throws IllegalArgumentException {
        if (Lexeme.length() == 1) {
            if (isInteger(Lexeme)) {
                Token token = new Token(Lexeme, Type.INT_LIT , row, column);
                tokens.add(token);

            } else if (Lexeme.equals("+")) {
                Token token = new Token(Lexeme, Type.ADDITION, row, column);
                tokens.add(token);

            }else if (Lexeme.equals("-")) {
                Token token = new Token(Lexeme, Type.SUBTRACTION, row, column);
                tokens.add(token);

            } else if (Lexeme.equals("*")) {
                Token token = new Token(Lexeme, Type.MULTIPLICATION, row, column);
                tokens.add(token);

            } else if (Lexeme.equals("/")) {
                Token token = new Token(Lexeme, Type.DIVISION, row, column);
                tokens.add(token);

            } else if (Lexeme.equals("(")) {
                Token token = new Token(Lexeme, Type.LEFT_PARENTH, row, column);
                tokens.add(token);
                
            } else if (Lexeme.equals(")")) {
                Token token = new Token(Lexeme, Type.RIGHT_PARENTH, row, column);
                tokens.add(token);

            }else if (Lexeme.equals(";")){
                Token token = new Token("EOS", Type.EOS_TOKEN, row, column);
                tokens.add(token);
